...
class MOVimmChar(val reg : Int, val movVal : Char) : Instr() // MOV r4, #'c'
...
class STRspMov(val reg : Int, val spMov : Int) : Instr() // e.g. STR r4, [sp, #-4]! decrements the stack pointer by 4 THEN stores into the stack at that location
class ADDS(val reg1: Int, val reg2: Int, val re3: Int) : Instr()
class ADD(val reg1: Int, val reg2: Int, val value: Int) : Instr()
class ADDspsp(val value: Int) : Instr()
