...
class AstAssemblyGenerator(
    ...
    private fun translateWhileBlock(node: WhileBlockAST) {
        ...
        // but only add the loop label
        // (we want the labels generated by the statements in the loop block to come before the
        // check block)
        val oldLabel = currentLabel
        val loopLabel = newBranchLabel()
        val checkLabel = newBranchLabel(addToText = false)
        val endLabel = newBranchLabel(addToText = false)
    ...
    private fun translateForBlock(node: ForBlockAST) {
        ...
        // Create the labels we need, in this precise correct order but only add the loop label
        // (we want the labels generated by the statements in the loop block to come before the
        // check block)
        val oldLabel = currentLabel
        val loopLabel = newBranchLabel(true)
        val updateLabel = newBranchLabel(false)
        val checkLabel = newBranchLabel(false)
        val endLabel = newBranchLabel(false)
